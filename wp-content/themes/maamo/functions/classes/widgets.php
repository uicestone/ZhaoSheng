...
class Padd_Widget_Subscribe extends WP_Widget {

	function Padd_Widget_Subscribe() {
		$widget_ops = array(
						'classname' => 'box-subscribe',
						'description' => PADD_THEME_NAME . ' Theme widget for subscribe line. For the management of subscription, go to ' . PADD_THEME_NAME . ' Options.'
					);
		$this->WP_Widget(PADD_THEME_SLUG . '_subscribe',PADD_THEME_NAME . ' Subscribe', $widget_ops);
		$this->alt_option_name = PADD_THEME_SLUG . '_widget_subscribe';
	}
	...
	function widget($args,$instance) {
		...
		if (!empty($title)) {
			...
			echo $before_title . 'Subscribe to My Blog' . $after_title . "\n";
		}
		padd_widget_funct_subscribe();
		echo $after_widget . "\n";
	}
...
class Padd_Widget_Feedburner extends WP_Widget {
	...
	function Padd_Widget_Feedburner() {
		$widget_ops = array(
						...
						'description' => PADD_THEME_NAME . ' Theme widget for Feedburner subscription form.'
					);
		$this->WP_Widget(PADD_THEME_SLUG . '_feedburner',PADD_THEME_NAME . ' Feedburner Subscription', $widget_ops);
	...
	function widget($args,$instance) {
		...
		if (!empty($title)) {
			...
			echo $before_title . 'Subscribe to RSS' . $after_title . "\n";
		}
		padd_widget_funct_feedburner();
		echo $after_widget . "\n";
	}
...
class Padd_Widget_Featured_Video extends WP_Widget {
	...
	function widget($args,$instance) {
		...
		padd_widget_funct_featured_video();
		echo $after_widget . "\n";
	}
...
register_widget('Padd_Widget_Subscribe');
